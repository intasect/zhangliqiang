...
/********************************************************************
* 功能名称：do_CompositeShopMasterPeradmModel_OnResponse
* 描述：请求返回之后重新绑定model里的数据
* 输入参数：eventObject
* 输出参数：无
*/
function do_CompositeShopMasterPeradmModel_OnResponse(eventObject)
{
...
function do_Form_InitDone(eventObject)
{
    ...
    WebForm.getLayoutElement(gpxCompositeShopMasterPeradm).style.height = "auto";
    WebForm.getLayoutElement(gpxCompositeShopMasterPeradm).className += "autoextent";
    gpxuserinfo.create(); 
   ...
   // gsShopId = "78e3b51c-e998-11e0-fa92-aea629981b2e";
    ...
    modelCol_Single["GetShopMasterPeradmModel"] = GetShopMasterPeradmModel;
...
function getCompositeObject(){
	    ...
	    CompositeShopMasterPeradmModel.clear();
	    CompositeShopMasterPeradmModel.setMethodRequest(getShopMasterObjectReq);
	    CompositeShopMasterPeradmModel.reset(); 
	    var  compositeData=CompositeShopMasterPeradmModel.getData(); 
 ...
 function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_MASTER_PERADM']"))
        {
            var shopMasterOperationDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopMasterPeradmModel,responseNode,"SHOP_MASTER_PERADM",shopMasterOperationDataNode);
        } 
 ...
 function do_btnexit_Click(eventObject)
 {
  ...
  /***********************************
  *  功能名称：do_CompositeShopMasterPeradmModel_OnSOAPFault();
  *  功能描述：SOAP请求失败时消息提示
  */
  function do_CompositeShopMasterPeradmModel_OnSOAPFault(eventObject)
  {
	insertOperation  = 0;
    ...
    {
      insertOperation = 1;   
      saveCompositeObject(CompositeShopMasterPeradmModel,modelCol_Single,modelCol_Multi);
      if(CompositeShopMasterPeradmModel.soapFaultOccurred) 
      { 
          return; 
      }
    }
    ...
    function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
    {
    	 switch(modelName)
         {             
             case "GetShopMasterPeradmModel":
             	fillValuesInGetShopMasterPeradmObjectModel(modelDataDoc ,modelBObjName,tupleNode);
             break;
         }
    }

    /********************************************************************
     * 功能名称	: fillValuesInGetShopMasterPeradmObjectModel();
     * 功能描述:  为 model增加状态位节点
     * 输入参数：modelData modelBObjName tupleNode
     * 输出参数：无
     */
    function fillValuesInGetShopMasterPeradmObjectModel(modelData ,modelBObjName,tupleNode)
    {  
