...
function initForm(eventObject)
{
	...
	WebForm.getLayoutElement(gpxShopComFitment).style.height = "auto";
    WebForm.getLayoutElement(gpxShopComFitment).className += "autoextent";
    ...
    //创建单条记录模型数组
    modelCol_Single["GetShopFitmentForenoticeObjectModel"] = GetShopFitmentForenoticeObjectModel;
...
function getCompositeObject()
{
    ...
    CompositeShopFitmentInfoModel.setMethodRequest(getCompleteXMLDocument);
    CompositeShopFitmentInfoModel.reset();
    var compositeData=CompositeShopFitmentInfoModel.getData();
...
{
	insertOperation = 1;
	saveCompositeObject(CompositeShopFitmentInfoModel,modelCol_Single,modelCol_Multi);
}
...
function bindDataToModels(responseNode)
{
        ...
        { 
         ...
         if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_FITMENT_FORENOTICE']"))
         {
             ...
             bindSingleTupleModel(GetShopFitmentForenoticeObjectModel,responseNode,"SHOP_FITMENT_FORENOTICE",shopInfoDataNode);
         }
...
function submitHandler(confirmReturnValue)
{
		if (confirmReturnValue == 1)
		{
			if(stepcode.getValue() == "05")
			{
				...
				if(CompositeShopFitmentInfoModel.soapFaultOccurred)
				{
					submitOperation = 0;
					return;
				}
