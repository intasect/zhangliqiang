...
function do_Form_Init(eventObject)
{
    ...
    gpxShopTransPosUserInfo.disable();
...
{
     WebForm.getLayoutElement(gpxCompositeShopTransPosModel).style.height = "auto";
     WebForm.getLayoutElement(gpxCompositeShopTransPosModel).className += "autoextent"; 
     gsShopId =shopid.getValue();
   //  gsShopId = "78e3b51c-e998-11e0-fbd6-c80a6a5d1f83";
     userArray=getUserInfo();
     gpxShopTransPosUserInfo.create();
     staticObject = getStaticValue();
     cordys.addDOMListener(File_RepositoryTable, "ondblclick",
 			completeDownloadFile);
     modelCol_Single["GetShopTransPosModel"] = GetShopTransPosModel;
...
function getCompositeObject(){
	  ...
	  CompositeShopTransPosModel.clear();
	  CompositeShopTransPosModel.setMethodRequest(getShopMasterObjectReq);
	  CompositeShopTransPosModel.reset(); 
	  var compositeData=CompositeShopTransPosModel.getData(); 
 ...
 function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_TRANSFER_POS']"))
        {
            var shopTransPosDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopTransPosModel,responseNode,"SHOP_TRANSFER_POS",shopTransPosDataNode);
        }
...
/***********************************
 *  功能名称：do_CompositeShopTransPosModel_OnSOAPFault();
 *  功能描述：SOAP请求失败时消息提示
 *  输入参数:eventObject
 *  输出参数：无
 */
 function do_CompositeShopTransPosModel_OnSOAPFault(eventObject)
 {
	insertOperation = 0;
 ...
 {
   insertOperation = 1;   
   saveCompositeObject(CompositeShopTransPosModel,modelCol_Single,modelCol_Multi);
   if(CompositeShopTransPosModel.soapFaultOccurred) 
   { 
       return; 
   }
 }
 ...
 function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
 {
 	 switch(modelName)
      {             
          case "GetShopTransPosModel":
          	fillValuesInGetShopTransPosObjectModel(modelDataDoc ,modelBObjName,tupleNode);
          break;
      }
 }
 
 /********************************************************************
  * 功能名称	: fillValuesInGetShopTransPosObjectModel();
  * 功能描述:  为 model增加状态位节点
  * 输入参数：modelData modelBObjName tupleNode
  * 输出参数：无
  */
 function fillValuesInGetShopTransPosObjectModel(modelData ,modelBObjName,tupleNode)
 {  
