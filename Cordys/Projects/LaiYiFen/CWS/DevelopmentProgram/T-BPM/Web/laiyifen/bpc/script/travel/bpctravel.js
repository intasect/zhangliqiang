...
function do_Form_InitDone(eventObject)
{
	...
	modelCol_Single["GetBpcTravelObjectModel"] = GetBpcTravelObjectModel;
	...
	modelCol_Multi["GetBpcFeedepObjectsModel"] = GetBpcFeedepObjectsModel;
	modelCol_Multi["GetBpcTravelItemObjectsModel"] = GetBpcTravelItemObjectsModel;
	modelCol_Multi["GetAttachmentModel"] = GetAttachmentModel;
	
	//放置需要必填项校验的output,select控件
	validateControls["is_fee_selfdep"] = is_fee_selfdep;
...
function getCompositeObject()
{
	...
	CompositeBPCTravelInfoModel.setMethodRequest(getCompleteXMLDocument);
	CompositeBPCTravelInfoModel.reset();
	var compositeData=CompositeBPCTravelInfoModel.getData();
...
{
	insertOperation = 1;
	saveCompositeObject(CompositeBPCTravelInfoModel,modelCol_Single,modelCol_Multi);
	insertOperation = 0;
}
...
function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
{
	switch(modelName)
	{
		...
		*/
		...
		case "GetBpcTravelItemObjectsModel" :
			createFormId(modelDataDoc ,modelBObjName,tupleNode);
		break;
...
function bindDataToModels(responseNode)
{
	if(insertOperation == 0)
	{
	   ...
	   { 
		...
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_TRAVEL']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindSingleTupleModel(GetBpcTravelObjectModel,responseNode,"BPC_TRAVEL",objDataNode);
		}
		
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_TRAVELDEP']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindMultiTupleModel(GetBpcFeedepObjectsModel,responseNode,"BPC_TRAVELDEP",objDataNode);
		}
		
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_TRAVEL_ITEM']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindMultiTupleModel(GetBpcTravelItemObjectsModel,responseNode,"BPC_TRAVEL_ITEM",objDataNode);
		}
...
function startWorkflow(){
	...
	if(Workflow.getProcessInstanceId()==undefined || Workflow.getProcessInstanceId()=="")
	{
		...
		var flowURL = "Business Process Models/com/laiyifen/bpc/travel/总部差旅报销流程" ;
		if (business_type.getValue() != "总部差旅报销流程") flowURL = "Business Process Models/com/laiyifen/bpc/travel/子公司差旅报销流程" ;
	...
	{ 
		if(CompositeBPCTravelInfoModel.soapFaultOccurred) 
		{ 
			return; 
		}
...
function do_CompositeModel_OnResponse(eventObject)
{
	...
	var tmpnode=cordys.selectXMLNode(responseNode ,".//*[local-name()='BPC_TRAVEL']/*[local-name()='FORM_ID']")
