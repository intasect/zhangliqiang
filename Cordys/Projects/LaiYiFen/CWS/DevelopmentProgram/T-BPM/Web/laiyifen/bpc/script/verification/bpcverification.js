...
function do_Form_InitDone(eventObject)
{
	...
	modelCol_Single["GetBpcVerificationObjectModel"] = GetBpcVerificationObjectModel;
	...
	modelCol_Multi["GetBpcVerificationItemObjectsModel"] = GetBpcVerificationItemObjectsModel;
...
function getCompositeObject()
{
	...
	CompositeBPCVerificationInfoModel.setMethodRequest(getCompleteXMLDocument);
	CompositeBPCVerificationInfoModel.reset();
	var compositeData=CompositeBPCVerificationInfoModel.getData();
...
{
	insertOperation = 1;
	saveCompositeObject(CompositeBPCVerificationInfoModel,modelCol_Single,modelCol_Multi);
	insertOperation = 0;
}
...
function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
{
	switch(modelName)
	{
		...
		*/
		...
		case "GetBpcVerificationItemObjectsModel" :
...
function bindDataToModels(responseNode)
{
	if(insertOperation == 0)
	{
	   ...
	   { 
		...
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_VERIFICATION']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindSingleTupleModel(GetBpcVerificationObjectModel,responseNode,"BPC_VERIFICATION",objDataNode);
		}
		
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_VERIFICATION_ITEM']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindMultiTupleModel(GetBpcVerificationItemObjectsModel,responseNode,"BPC_VERIFICATION_ITEM",objDataNode);
		}
...
function startWorkflow(){
	...
	if(Workflow.getProcessInstanceId()==undefined || Workflow.getProcessInstanceId()=="")
	{
		...
		var flowURL = "Business Process Models/com/laiyifen/bpc/verification/总部借款核销流程" ;
		if (business_type.getValue() != "总部借款核销流程") flowURL = "Business Process Models/com/laiyifen/bpc/verification/子公司借款核销流程" ;
	...
	{ 
		if(CompositeBPCVerificationInfoModel.soapFaultOccurred) 
		{ 
			return; 
		}
...
function do_CompositeModel_OnResponse(eventObject)
{
	...
	var tmpnode=cordys.selectXMLNode(responseNode ,".//*[local-name()='BPC_VERIFICATION']/*[local-name()='FORM_ID']")
