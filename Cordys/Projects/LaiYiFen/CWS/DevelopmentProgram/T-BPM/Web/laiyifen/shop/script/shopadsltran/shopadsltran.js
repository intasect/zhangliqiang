...
//用户在其任务完成时,只能查看表单内容,而不能修改表单内容
    ...
    gpxShopAdsltranUserInfo.disable();
...
function do_Form_InitDone(eventObject) { 
    WebForm.getLayoutElement(gpxCompositeShopAdsltranModel).style.height = "auto";
    WebForm.getLayoutElement(gpxCompositeShopAdsltranModel).className += "autoextent";
    gsShopId = shopid.getValue();
  // gsShopId = "78e3b51c-e998-11e0-fbd6-c80a6a5d1f83";
    userArray=getUserInfo();
    gpxShopAdsltranUserInfo.create();
    staticObject = getStaticValue();
    cordys.addDOMListener(File_RepositoryTable, "ondblclick",
			completeDownloadFile);
    modelCol_Single["GetShopAdsltranModel"] = GetShopAdsltranModel;
...
function getCompositeObject(){
	 ...
	 var appData =  application.event.data;
	    ...
	    CompositeShopAdsltranModel.clear();
	    CompositeShopAdsltranModel.setMethodRequest(getShopMasterObjectReq);
	    CompositeShopAdsltranModel.reset(); 
	    var compositeData=CompositeShopAdsltranModel.getData(); 
...
function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_ADSLTRAN']"))
        {
            var shopAdsltranDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopAdsltranModel,responseNode,"SHOP_ADSLTRAN",shopAdsltranDataNode);
        }
...
/***********************************
 *  功能名称：do_CompositeShopAdsltranModel_OnSOAPFault();
 *  功能描述：SOAP请求失败时消息提示
 *  输入参数:eventObject
 *  输出参数：无
 */
 function do_CompositeShopAdsltranModel_OnSOAPFault(eventObject)
 {
	 insertOperation = 0;
 ...
 {
   insertOperation = 1;   
   saveCompositeObject(CompositeShopAdsltranModel,modelCol_Single,modelCol_Multi);
   if(CompositeShopAdsltranModel.soapFaultOccurred) 
   { 
       return; 
   }
 }
 ...
 function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
 {
 	 switch(modelName)
      {             
          case "GetShopAdsltranModel":
          	fillValuesInGetShopAdslObjectModel(modelDataDoc ,modelBObjName,tupleNode);
          break;
      }
 }
 
 /********************************************************************
  * 功能名称	: fillValuesInGetShopAdslObjectModel();
  * 功能描述:  为 model增加状态位节点
  * 输入参数：modelData modelBObjName tupleNode
  * 输出参数：无
  */
 function fillValuesInGetShopAdslObjectModel(modelData ,modelBObjName,tupleNode)
 {  
