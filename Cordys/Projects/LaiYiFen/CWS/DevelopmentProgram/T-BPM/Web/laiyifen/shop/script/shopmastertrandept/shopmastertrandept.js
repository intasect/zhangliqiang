...
{
    WebForm.getLayoutElement(gpxCompositeShopMasterTrandept).style.height = "auto";
    WebForm.getLayoutElement(gpxCompositeShopMasterTrandept).className += "autoextent";
    ...
    modelCol_Single["GetShopMasterTrandeptModel"] = GetShopMasterTrandeptModel;
...
function getCompositeObject(){
	...
	var appData = application.event.data;
	 if(appData && appData.shopid)
	 {
	    ...
	    CompositeShopMasterTrandeptModel.clear();
	    CompositeShopMasterTrandeptModel.setMethodRequest(getShopMasterObjectReq);
	    CompositeShopMasterTrandeptModel.reset(); 
	    var compositeData=CompositeShopMasterTrandeptModel.getData(); 
 ...
 function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_MASTER_TRANDEPT']"))
        {
            var shopMasterOperationDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopMasterTrandeptModel,responseNode,"SHOP_MASTER_TRANDEPT",shopMasterOperationDataNode);
        }
 ...
 * 输入参数：eventObject
  * 输出参数：无
 */
  function do_imgprovidedc_Click(eventObject)
  {
      getDialogValue(eventObject,providedcode,providedc,"WERKS_DC","供应DC列表","320px","230px","1");
  }
...
/********************************************************************
   ...
   * 输出参数：无
   */
  function do_imgregionaltransportation_Click(eventObject)
  {
      getDialogValue(eventObject,regionaltransportation,regionaltransportationname,"TRANSPZONE","运输区域列表","320px","230px","1");
  } 
  
  /***********************************
  *  功能名称：do_CompositeShopMasterTrandeptModel_OnSOAPFault();
  *  功能描述：SOAP请求失败时消息提示
  *  输入参数:eventObject
  *  输出参数：无
  */
  function do_CompositeShopMasterTrandeptModel_OnSOAPFault(eventObject)
  {
	insertOperation=0;
    ...
    {
      insertOperation = 1;   
      saveCompositeObject(CompositeShopMasterTrandeptModel,modelCol_Single,modelCol_Multi);
      if(CompositeShopMasterTrandeptModel.soapFaultOccurred) 
      { 
          return; 
      }
    }
    ...
    function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
    {
    	 switch(modelName)
         {             
             case "GetShopMasterTrandeptModel":
            	 fillValuesInGetShopMasterTrandeptObjectModel(modelDataDoc ,modelBObjName,tupleNode);
             break;
         }
    }

    /********************************************************************
     * 功能名称	: fillValuesInGetShopMasterTrandeptObjectModel();
     * 功能描述:  为 model增加状态位节点
     * 输入参数：modelData modelBObjName tupleNode
     * 输出参数：无
     */
    function fillValuesInGetShopMasterTrandeptObjectModel(modelData ,modelBObjName,tupleNode)
    {  
    ...
    function check(){
    	var flag = true;
        var prodc = providedc.getValue();
        if(prodc==null||prodc==""){
    		  flag = false;
    		  application.notify("供应DC不能为空！");
    		  providedc.focus();
    		  return false;
    		     
        }  
        
        var  regionalname  = regionaltransportationname.getValue();
        if(regionalname==null||regionalname==""){
  		  flag = false;
  		  application.notify("运输区域不能为空！");
  		 regionaltransportationname.focus();
  		  return false;
