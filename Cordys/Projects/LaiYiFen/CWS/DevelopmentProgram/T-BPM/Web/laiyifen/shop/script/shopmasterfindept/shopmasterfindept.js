...
{
    WebForm.getLayoutElement(gpxCompositeShopMasterFindept).style.height = "auto";
    WebForm.getLayoutElement(gpxCompositeShopMasterFindept).className += "autoextent";
   ...
   // gsShopId = "78e3b51c-e998-11e0-fa92-aea629981b2e";
    ...
    modelCol_Single["GetShopMasterFindeptModel"] = GetShopMasterFindeptModel;
...
function getCompositeObject(){
	 ...
	 if(appData && appData.shopid)
	 {
	  ...
	  CompositeShopMasterFindeptModel.clear();
	  CompositeShopMasterFindeptModel.setMethodRequest(getShopMasterObjectReq);
	  CompositeShopMasterFindeptModel.reset(); 
	  var compositeData=CompositeShopMasterFindeptModel.getData(); 
 ...
 function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_MASTER_FINDEPT']"))
        {
            var shopMasterOperationDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopMasterFindeptModel,responseNode,"SHOP_MASTER_FINDEPT",shopMasterOperationDataNode);
          
        }else{
        	thirdmarkname.setValue("1");
        }
 ...
 function closeHandler(confirmReturnValue)
 {
  ...
  /********************************************************************
   * 功能名称	: do_imgprovidedc_Click();
  * 描述: 调用第三方结算表示列表弹出页
 * 输入参数：eventObject
  * 输出参数：无
 */
  function do_imgthirdmarkname_Click(eventObject)
  {
      getDialogValue(eventObject,thirdmarkcode,thirdmarkname,"ZTHRD","第三表结算标示列表","320px","230px","1");
  }


/********************************************************************
   * 功能名称	: do_imgaccountbank_Click();
   * 描述: 调用运输区域列表弹出页
   * 输入参数：eventObject
   * 输出参数：无
   */
  function do_imgaccountbank_Click(eventObject)
  {
      getDialogValue(eventObject,accountbankcode,accountbank,"ACCOUNT_BANK","开户行列表","320px","230px","1");
  } 

/***********************************
*  功能名称：do_CompositeShopMasterFindeptModel_OnSOAPFault();
*  功能描述：SOAP请求失败时消息提示
*  输入参数:eventObject
*  输出参数：无
*/
function do_CompositeShopMasterFindeptModel_OnSOAPFault(eventObject)
{
	insertOperation=0;
  ...
  function do_btnexit_Click(eventObject)
  {
    ...
    {
      insertOperation = 1;   
      saveCompositeObject(CompositeShopMasterFindeptModel,modelCol_Single,modelCol_Multi);
      if(CompositeShopMasterFindeptModel.soapFaultOccurred) 
      { 
          return; 
      }
    }
    ...
    function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
    {
    	 switch(modelName)
         {             
             case "GetShopMasterFindeptModel":
             	fillValuesInGetShopMasterFindeptObjectModel(modelDataDoc ,modelBObjName,tupleNode);
             break;
         }
    }

   /********************************************************************
     * 功能名称	: fillValuesInGetShopMasterFindeptObjectModel();
     * 功能描述:  为 model增加状态位节点
     * 输入参数：modelData modelBObjName tupleNode
     * 输出参数：无
     */
    function fillValuesInGetShopMasterFindeptObjectModel(modelData ,modelBObjName,tupleNode)
    {  
        ...
        if(statusNode!=null){
        	...
        	statusNode = cordys.createElementNS(modelDataDoc,"http://schemas.cordys.com/tbpm/shop","STATUS");
            ...
            else
            {
          	...
          	cordys.appendXMLNode(statusNode, cordys.selectXMLNode(parentNewNode,".//*[local-name()='" + modelBObjName + "']"));
        }
    ...
    function check(){
    	var flag = true;
    	/**
        var accbank = accountbank.getValue();
        if(accbank==null||accbank==""){
    		  flag = false;
    		  application.notify("开户行不能为空！");
    		  accountbank.focus();    		     
        }   
    	 */     
        return flag;
    ...
    function setShopMasterValue(){
    	accountbankcode.setValue(iptSapNo.getValue());
    }
