   ...
   if(oParam && oParam.status && oParam.status!=""){
	...
	if(stepcode.getValue()=="" || stepcode.getValue()=="05"){
	   ...
	   if(gIsFlag==true && (gStatus=="1"||gStatus=="2")){
          ...
          gpxbranchmanageapp.disable();
	      tbbbpmmonitor.hide();
		  tbbreturn.hide();
		  tbbsuspend.hide();
		  tbbsave.hide();
		  tbbsubmit.hide();
		  gpxshopestimation.disable();
		  gpxcommon.disable();
	   }else{
	   ...
	   gpxbranchmanageapp.show();
       gpxbranchmanageapp.enable();
	   gpxbranchmanageapp.expand();
       gpxshopcircle.disable();
	   gpxanalyseinfo.disable();
	   gpxsuggestion.disable();
	   gpxshopinfo.disable();
	   ...
	   gpxsuggestion.disable();
       gpxbranchmanageapp.disable();
	   gpxbranchmanageapp.collapse();
	}
   gpxuserinfo.disable();
   gpxhide.hide();
   gpxshopmaster.hide();
   shopestimation.hideColumn(7);
   gpxshopestimation.expand();
...
function do_Form_InitDone(eventObject)
{
	...
	validateControls["supermaturity"] = supermaturity;
	...
	validateControls["innerdoorwith"] = innerdoorwith;
...
function do_toolbutton_return_Click(eventObject)
{
	  //弹出驳回确认框
      popupEvent = eventObject;
...
/********************************************************************
 * 功能名称	: do_gpxshopcircle_OnExpand();
 * 描述: 展开工程groupbox时判断是否需要创建新记录
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_gpxshopcircle_OnExpand(eventObject)
{
...
/********************************************************************
 * 功能名称	: do_gpxbranchmanageapp_OnExpand();
 * 描述: 展开工程groupbox时判断是否需要创建新记录
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_gpxbranchmanageapp_OnExpand(eventObject)
{
		...
		createBusinessObjOnModel(GetShopTargetCheckObjectModel,gpxbranchmanageapp,"SHOP_TARGET_CHECK");
}
/********************************************************************
...
function do_gpxanalyseinfo_OnExpand(eventObject)
{
		...
		do_isnearshop_Change(eventObject);
...
{ 
	...
	do_innerdoorwith_Change(eventObject);
} 


function do_gpxshopestimation_OnExpand(eventObject)
{
    shopestimation.hideColumn(7);
}
...
function fillValuesInGetShopTargetInfoObjectModel(modelData ,modelBObjName,tupleNode)
{
	...
	cordys.setTextContent(formTypeNode,"1");
...
function bindDataToModels(responseNode)
{
		...
		if(stepcode.getValue()=="15"||(gIsFlag==true && (gStatus=="1"||gStatus=="2"))){
			var shoptargetChkNode = cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_TARGET_CHECK']");
			if(shoptargetChkNode)
			{
				var shopCheckDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
				bindSingleTupleModel(GetShopTargetCheckObjectModel,responseNode,"SHOP_TARGET_CHECK",shopCheckDataNode);
			}
		}
		...
		if(!insertOperation){
            var shopTargetCircleInfo = cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_TARGET_CIRCLE_INFO']");
			if(shopTargetCircleInfo)
			{
				var busCircleDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
				bindSingleTupleModel(GetShopTargetCircleInfoObjectModel,responseNode,"SHOP_TARGET_CIRCLE_INFO",busCircleDataNode);
			}
			var shopTargetAssignMntInfo  = cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_TARGET_ASSESMENT']");
			if(shopTargetAssignMntInfo)
			{
				var assesmentDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
				bindSingleTupleModel(GetShopTargetAssesmentObjectModel,responseNode,"SHOP_TARGET_ASSESMENT",assesmentDataNode);
			}
		}  
...
function submitTask()
{
   ...
   if(typeof(Workflow.getProcessInstanceId())=='undefined' || Workflow.getProcessInstanceId()==null)
   {
	   ...
	   cordys.setNodeText(request,".//*[local-name()='ProcessName']","新开店主流程");
       cordys.setNodeText(request,".//*[local-name()='DecisionAttribute']","00");
	   cordys.setNodeText(request,".//*[local-name()='Ext3']","1");
...
{
   getDialogValue(eventObject,isfullresourcecode,isfullresource,"FULLRESOURCE","办证资料列表","320px","230px","*");
}
...
function do_imgprovince_Click(eventObject){
  getDialogValue(eventObject,shopareacode,provicedec,"BLAND","省列表","320px","230px","1");
}

/********************************************************************
 * 功能名称	: do_imgleasefitment_Click();
 * 描述: 弹出框,选择租赁性质列表
 * 输入参数：无
 * 输出参数：无
 */
function do_imgleasefitment_Click(eventObject)
{
    getDialogValue(eventObject,"",isleasefitmentdec,"ZMDLX","租赁性质列表","320px","230px","1");
}

/********************************************************************
 * 功能名称	: do_imgsupermaturity_Click();
 * 描述: 弹出框,选择商超整体成熟度列表
 * 输入参数：无
 * 输出参数：无
 */
function do_imgsupermaturity_Click(eventObject)
{
   getDialogValue(eventObject,supermaturitycode,supermaturity,"SUPER_MATURITY","商超整体成熟度列表","320px","230px","1");
}

/********************************************************************
 * 功能名称	: do_imgshopattrname_Click();
 * 描述: 弹出框,选择房产性质列表
 * 输入参数：无
 * 输出参数：无
 */
function do_imgshopattrname_Click(eventObject)
{
   getDialogValue(eventObject,houseattr,houseproperty,"SUPER_HOUSE_PROPERTY","房产性质列表","320px","230px","1");
}

/********************************************************************
 * 功能名称	: do_imghouseattrname_Click();
 * 描述: 弹出框,选择房东性质列表
 * 输入参数：无
 * 输出参数：无
 */
function do_imghouseattrname_Click(eventObject)
{ 
   getDialogValue(eventObject,vendorno,landlord,"LANDLORD","房东性质列表","320px","230px","1");
}

/********************************************************************
 * 功能名称	: do_imglivingqualitydec_Click();
 * 描述:调用住宅区生活水平列表弹出页
 * 输入参数：无
 * 输出参数：无
 */
function do_imglivingqualitydec_Click(eventObject)
{
...
function setShopMasterValue(){
	...
	if(shopDetailAddress.length>60){
	  application.notify("门店地址已超过60个字符,请精减相关信息.门店详细地址是通过省,市,区,路,门号组成.");
	  isFlag=false;
	}
    ...
    useareamaster.setValue(usearea.getValue());
	...
	if(stepcode.getValue()=="15"){
	   var rowsCollection=shopestimation.getRows();
	   var junetosept="";
	   var octtomay="";
       ...
       for (var i=0;i<rowsCollection.length; i++) {
			var stepscodeObj=cordys.getElementById(rowsCollection[i],"circlecode");
...
/********************************************************************
 * 功能名称	: do_isrentincreased_Change();
 * 描述: 后仓是否有决定如何使用控件是否显示
 * 输入参数：eventObject
 * 输出参数：无
 */
function  do_innerdoorwith_Change(eventObject){
   if(innerdoorwith.getValue() == "1")
    {
        doorwitharea.show();
		doorwitharea.enable();
    }
    ...
    {
        doorwitharea.disable();
        doorwitharea.setValue("");
    }	
}

/********************************************************************
 * 功能名称	: do_ismaturearea_Change();
 * 描述: 是否是成熟区决定如何使用控件是否显示
 * 输入参数：eventObject
 * 输出参数：无
 */
 /*
function do_ismaturearea_Change(eventObject){
	if(innerdoorwith.getValue() == "1")
    {
        totalapartments.show();
		totalapartments.enable();
    }
    else
    {
        totalapartments.disable();
        totalapartments.setValue("");
    }	
}
*/
/********************************************************************
 * 功能名称	: do_isnearshop_Change(eventObject);
 * 描述: 周边有无来伊份门店决定如何使用控件是否显示
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_isnearshop_Change(eventObject){
    if(isnearshop.getValue() == "1")
    {
        oldaddress.show();
		oldaddress.enable();
		oldoperating.show();
...
/********************************************************************
 * 功能名称	: do_hasopponent_Change(eventObject);
 * 描述: 有无竞争对手决定如何使用控件是否显示
 * 输入参数：eventObject
 * 输出参数：无
 */
...
/********************************************************************
 * 功能名称	: do_navInsert1_AfterInsert();
 * 描述: Table控件插入行后赋值
 * 输入参数：无
 * 输出参数：无
 */
function do_navInsert1_AfterInsert(eventObject)
{
    var rowInx = shopestimation.getIndex();
        operatoprole[rowInx].setValue(userArray.roleName);
        operatorname[rowInx].setValue(userArray.userName);
		circlecode[rowInx].setValue(stepcode.getValue());
        navInsert1.disable();
}
...
/********************************************************************
 * 功能名称	: do_shopestimation_OnSelectRow();
 * 描述: 控制Table控件内容是否可修改
 * 输入参数：无
 * 输出参数：无
 */
function do_shopestimation_OnSelectRow(eventObject)
{
    var rowInx = shopestimation.getIndex();
    var stepCode=circlecode[rowInx].getValue();
...
function do_deleteestimation_BeforeDelete(eventObject){
    var rows=shopestimation.getCheckedRows();
    for(var i = 0;i < rows.length;i++)
    {
        var rowInx = rows[i].sectionRowIndex;
        var stepsCode = circlecode[rowInx].getValue();
...
function do_validation_Charge()
{
    ...
    if(userInfoByRoleNode)
    {
	  var data = new Object();
          ...
          application.showDialog(dialogSelectCharge.XMLDocument.documentElement,data,null,GetSelectReturnValue,true);
	}else{
	   submitTask();
       submitOperation =0;
	}
...
function do_junetosept_InFocus(eventObject){
   if(rent.getValue()=="" && rentperiod.getValue()=="" && propertymanagementfees.getValue()=="" && agencyfees.getValue()=="" && transferfee.getValue()==""){
	   application.notify("请选输入租金,租房期限,物业管理费,中介费,进场费");
	   return;
   }
}

/********************************************************************
 * 功能名称	: do_gpxcommon_OnExpand(eventObject);
 * 描述:绑定审批历史记录
 * 输入参数：eventObject
 * 输出参数：无
 */
