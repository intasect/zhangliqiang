...
{
    WebForm.getLayoutElement(gpxCompositeShopEquacc).style.height = "auto";
    WebForm.getLayoutElement(gpxCompositeShopEquacc).className += "autoextent";
  ...
  // gsShopId = "78e3b51c-e998-11e0-fa92-aea629981b2e";
    ...
    modelCol_Single["GetShopEquaccModel"] = GetShopEquaccModel;
...
function getCompositeObject(){	
    ...
    CompositeShopEquaccModel.clear();
    CompositeShopEquaccModel.setMethodRequest(getShopMasterObjectReq);
    CompositeShopEquaccModel.reset(); 
    var compositeData=CompositeShopEquaccModel.getData(); 
 ...
 function bindDataToModels(responseNode)
{
       ...
       if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_EQUACC']"))
        {
            var shopEquaccDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
            bindSingleTupleModel(GetShopEquaccModel,responseNode,"SHOP_EQUACC",shopEquaccDataNode);
        }
...
/***********************************
*  功能名称：do_CompositeShopEquaccModel_OnSOAPFault();
*  功能描述：SOAP请求失败时消息提示
*  输入参数：eventObject
*  输出参数：无
*/
function do_CompositeShopEquaccModel_OnSOAPFault(eventObject)
{
       insertOperation = 0;
...
function do_btnexit_Click(eventObject)
{
   ...
   else
  {
   ...
   /********************************************************************
   * 功能名称	: do_isusebalance_Change();
   * 描述:控制按钮，文本框的内容是否可输入
 * 输入参数：eventObject
   * 输出参数：无
   */

function do_isusebalance_Change(){
         var isselected = isusebalance.getValue();
         if(isselected == "否") {
           electscalesno.disable();
           electscalesno.setValue("");
         }else{
            electscalesno.enable();   
        }
}
...
{
  insertOperation = 1;   
  saveCompositeObject(CompositeShopEquaccModel,modelCol_Single,modelCol_Multi);
  if(CompositeShopEquaccModel.soapFaultOccurred) 
  { 
      return; 
  }
}
...
function fillBusiValueOnModels(modelName,modelDataDoc,modelBObjName,tupleNode)
{
	 switch(modelName)
     {             
         case "GetShopEquaccModel":
         	fillValuesInGetEquaccObjectModel(modelDataDoc ,modelBObjName,tupleNode);
         break;
     }
}

/********************************************************************
 * 功能名称	: fillValuesInGetEquaccObjectModel();
 * 功能描述:  为 model增加状态位节点
 * 输入参数：modelData modelBObjName tupleNode
 * 输出参数：无
 */
function fillValuesInGetEquaccObjectModel(modelData ,modelBObjName,tupleNode)
{  
...
function check(){
	var flag = true;
	var isbalance = isusebalance.getValue();
    if(isbalance==null||isbalance==""){
    	 flag = false;
    	 application.notify("请选择是否使用PC秤！");
    	 return false;
    }
