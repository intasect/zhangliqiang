...
function do_Form_InitDone(eventObject)
{
	...
	modelCol_Single["GetBpcFeeObjectModel"] = GetBpcFeeObjectModel;
	...
	modelCol_Multi["GetBpcFeeItemObjectsModel"] = GetBpcFeeItemObjectsModel;
...
{
	insertOperation = 1;
	saveCompositeObject(CompositeBPCFeeInfoModel,modelCol_Single,modelCol_Multi);
	insertOperation = 0;
}
	...
	if(insertOperation == 0)
	{
	   ...
	   { 
		...
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_FEE']"))
		{
			...
			bindSingleTupleModel(GetBpcFeeObjectModel,responseNode,"BPC_FEE",objDataNode);
		}
		
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_FEEDEP']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindMultiTupleModel(GetBpcFeedepObjectsModel,responseNode,"BPC_FEEDEP",objDataNode);
		}
		
		if(cordys.selectXMLNode(responseNode,".//*[local-name()='BPC_FEE_ITEM']"))
		{
			var objDataNode = cordys.cloneXMLDocument(dataXML.XMLDocument);
			bindMultiTupleModel(GetBpcFeeItemObjectsModel,responseNode,"BPC_FEE_ITEM",objDataNode);
		}
...
function startWorkflow(){
	...
	if(Workflow.getProcessInstanceId()==undefined || Workflow.getProcessInstanceId()=="")
	{
		...
		var flowURL = "Business Process Models/com/laiyifen/bpc/fee/总部报销流程" ;
		if (business_type.getValue() != "总部报销流程") flowURL = "Business Process Models/com/laiyifen/bpc/fee/子公司报销流程" ;
	...
	{ 
		if(CompositeBPCFeeInfoModel.soapFaultOccurred) 
		{ 
			return; 
		}
...
function mycheck(){
	...
	if(Workflow.getProcessInstanceId()!=null){
		...
		if( (business_type.getValue()=="总部报销流程" && step=="25") || (business_type.getValue()=="子公司报销流程" && step=="20" ) ){
			for(i=0;i<tbitemsinfo.getRows().length;i++){
				if(subject_code[i+1].getValue()==""){
				flag = false;
				application.notify("请选择科目编码、科目名称！");
				break;
				}
			}
			return flag;
