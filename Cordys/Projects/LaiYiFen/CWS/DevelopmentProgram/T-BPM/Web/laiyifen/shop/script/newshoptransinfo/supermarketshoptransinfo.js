...
function do_Form_InitDone(eventObject)
{
	...
	validateControls["isconfirmopen"] = isconfirmopen;
...
function bindDataToModels(responseNode)
{
	...
	if(cordys.selectXMLNode(responseNode,".//*[local-name()='SHOP_TRANS_INFO']"))
	{
		...
		if(formType == null)
		{
			formtype.setValue("1");	//formType=1表示是商超门店
...
/********************************************************************
 * 功能名称	: do_isconfirmopen_Change();
 * 描述: 根据是否有开张日期来决定开张日期控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_isconfirmopen_Change(eventObject)
{
	if(isconfirmopen.getValue() == "1")
	{
		openingdate.enable();
	}
	else
...
/********************************************************************
 * 功能名称	: do_isconfirmopen_Change();
 * 描述: 根据是否有消费卡来决定卡名控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_isusecard_Change(eventObject)
{
	if(isusecard.getValue() == "1")
	{
		unioncard.enable();
	}
	else
...
/********************************************************************
 * 功能名称	: do_hasprocedure_Change();
 * 描述: 根据施工队进场是否需要手续来决定控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_hasprocedure_Change(eventObject)
{
	if(hasprocedure.getValue() == "1")
	{
		decorationcontact.enable();
		decorationtrans.enable();
	}
	else
	{
		decorationcontact.disable();
		decorationcontact.setValue("");
		decorationtrans.disable();
		decorationtrans.setValue("");
	}
}

/********************************************************************
 * 功能名称	: do_workhasprocedure_Change();
 * 描述: 根据员工上班是否需要手续来决定控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_workhasprocedure_Change(eventObject)
{
	if(workhasprocedure.getValue() == "1")
	{
		workhandle.enable();
		worktime.enable();
	}
...
/********************************************************************
 * 功能名称	: do_waterlocation_Change();
 * 描述: 根据水表位置是否确认来决定控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_waterlocation_Change(eventObject)
{
    if(waterlocation.getValue() == "1")
    {
        waterdescription.enable();
    }
    else
    {
        waterdescription.disable();
        waterdescription.setValue("");
    }
}

/********************************************************************
 * 功能名称	: do_electroniclocation_Change();
 * 描述: 根据电表位置是否确认来决定控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_electroniclocation_Change(eventObject)
{
	if(electroniclocation.getValue() == "1")
	{
		electronicdesc.enable();
	}
	else
	{
		electronicdesc.disable();
		electronicdesc.setValue("");
	}
}

/********************************************************************
 * 功能名称	: do_isphoneapply_Change();
 * 描述: 根据电话是否可以安装来决定控件是否可编辑
 * 输入参数：eventObject
 * 输出参数：无
 */
function do_isphoneapply_Change(eventObject)
{
	if(isphoneapply.getValue() == "1")
	{
		phoneapply.enable();
	}
	else
